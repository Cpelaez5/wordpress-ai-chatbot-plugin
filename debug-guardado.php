<?php
/**
 * Script de debug para verificar el guardado de configuraciones
 */
...
echo "<h2>Debug de Guardado - Chatbot IA DeepSeek</h2>";
...
echo "<h3>2. Probar Guardado Manual Directo:</h3>";
...
echo "<h3>4. Probar Guardado via WordPress Settings API:</h3>";

// Simular el guardado via WordPress Settings API
$sanitized_key = $plugin_instance->sanitize_api_key($test_api_key);
$wp_saved = update_option('chatbot_ia_api_key', $sanitized_key);
echo "<p>✅ Guardado via Settings API: " . ($wp_saved ? 'EXITOSO' : 'FALLÓ') . "</p>";

// Verificar que se guardó
$wp_saved_key = get_option('chatbot_ia_api_key', '');
echo "<p>✅ Verificación Settings API: " . (empty($wp_saved_key) ? 'NO GUARDADA' : 'GUARDADA (' . strlen($wp_saved_key) . ' caracteres)') . "</p>";
...
echo "<h3>6. Verificar Estado Final:</h3>";
...
if (empty($final_key)) {
    ...
    $decrypted = $plugin_instance->decrypt_api_key($final_key);
...
echo "<h3>7. Probar Carga de Opciones:</h3>";
